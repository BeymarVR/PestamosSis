...
<a href="{{ route('admin.solicitud-credito.index') }}" 
    ...
    Logs
           ...
           <a href="{{ route('admin.nist.index') }}" class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('admin.nist.*') ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-700' : 'text-slate-700 hover:bg-slate-50 hover:text-slate-900' }}">
    
    <!-- Icono Escudo / NIST -->
    <svg class="mr-3 w-5 h-5 {{ request()->routeIs('admin.nist.*') ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-500' }}" 
         fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
